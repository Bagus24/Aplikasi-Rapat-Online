...
<div class="main-card mb-3 card">
    <div class="card-body">
        ...
        <br>
        <form class="" method="POST" action="{{ route('profil.update', $peserta->id) }}">
            @csrf
            @method('PUT')
            <div class="position-relative row form-group">
                <label for="exampleEmail" class="col-sm-2 col-form-label">NIPY</label>
                <div class="col-sm-10">
                    <input id="nipy" disabled class="form-control @error('nipy') is-invalid @enderror" value="{{ $peserta->nipy }}" required autofocus>
            ...
            <div class="position-relative row form-group">
                ...
                <div class="col-sm-10">
                    <input maxlength="40" placeholder="masukan status peserta.." id="status" type="text" class="form-control @error('status') is-invalid @enderror" name="status" value="{{ $peserta->status }}" required autofocus>
                    @error('status')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
...
<div class="main-card mb-3 card">
    <div class="card-body">
        ...
        <br>
        <form class="" method="POST" action="{{ url('profil/update-foto', $peserta->id) }}" enctype="multipart/form-data">
            @csrf
            <div class="position-relative row form-group">
